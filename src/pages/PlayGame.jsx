...
import { getGame, getMinigame } from '../lib/gameData'
...
export default function PlayGame() {
  ...
  const { user } = useAuth()
  const game = getGame(gameId) || getMinigame(gameId)
      ...
      <div className="max-w-md mx-auto px-4 py-12">
        ...
        <div className="brutalist-card p-8 text-center">
          ...
          <p className="text-navy/50 text-sm mb-6 font-medium">
            These games use AI image generation. Enter your Fal.ai API key to play. Stored locally only.
          </p>
          <form onSubmit={handleSetKey} className="space-y-3">
            <input type="password" placeholder="Enter your Fal.ai API key..."
          ...
          <p className="text-navy/40 text-xs mt-4 font-medium">Get your key at <span className="text-primary font-bold">fal.ai</span></p>
        </div>
      </div>
    )
  }

  // Render real minigame components
  const MinigameComponent = MINIGAME_COMPONENTS[gameId]
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {MinigameComponent ? (
        <MinigameComponent game={game} />
      ) : (
        <>
          {/* Original placeholder for old games */}
          <div
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            {['Hint', 'Skip', 'Submit'].map((action) => (
              <button key={action} className="py-3 bg-white text-navy text-sm font-black uppercase hover:bg-surface-light transition-colors border-2 border-navy">{action}</button>
            ))}
          </div>
